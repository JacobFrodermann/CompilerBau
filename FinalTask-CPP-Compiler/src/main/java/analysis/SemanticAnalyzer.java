...
public class SemanticAnalyzer {
      ...
      case Declaration.VarDef var -> {
        ...
        if (!typesMatch(var.type(), exprType)) {
          throw new SemanticException(
              "Type mismatch in variable definition: expected "
                  + var.type().name()
                  + ", got "
                  + exprType.name());
        }
  ...
  private void analyzeFunctionDef(Declaration.FunctionDef func) throws SemanticException {
    ...
    for (Parameter param : func.parameters()) {
      Symbol.VariableSymbol paramSymbol =
          new Symbol.VariableSymbol(param.name(), param.type(), param.isReference());
      funcScope.define(paramSymbol);
    }

    // Body analysieren
    // hier direkt die statements aus dem body weil wir sonst in der andereen methode nochmal ein
    // neuen scope machen würden was wir nicht wollen
    ...
    switch (stmt) {
      ...
      case Statement.VarDecl var -> {
        ...
        Symbol.VariableSymbol symbol =
            new Symbol.VariableSymbol(var.name(), var.type(), var.isReference());
      ...
      case Statement.VarDef var -> {
        ...
        if (!typesMatch(var.type(), exprType)) {
          throw new SemanticException(
              "Type mismatch: expected " + var.type().name() + ", got " + exprType.name());
        }
          ...
          if (!typesMatch(currentFunctionReturnType, retType)) {
            throw new SemanticException(
                "Return type mismatch: expected "
                    + currentFunctionReturnType.name()
                    + ", got "
                    + retType.name());
          }
        }
  ...
  public Type analyzeExpression(Expression expr) throws SemanticException {
    return switch (expr) {
      ...
      case Expression.CharLiteral lit -> new Type("char");
      ...
      case Expression.BinaryOp binOp -> {
        ...
        if (op.equals("+")) {
          if (!((leftType.name().equals("int") && rightType.name().equals("int"))
              || (leftType.name().equals("string") && !rightType.name().equals("string")))) {
            throw new SemanticException(
                "Arithmetic operators require int operands3 got "
                    + leftType.name()
                    + " and "
                    + rightType.name());
          }
      ...
      default -> {
        throw new SemanticException("shouldnt happen");
      }
    };
  }

  private Type analyzeFunctionCall(String name, List<Expression> arguments)
    ...
    if (func == null) {
      throw new SemanticException(
          "Function not found: "
              + name
              + " with signature "
              + new Signature(name, argTypes, argRefs));
    }
