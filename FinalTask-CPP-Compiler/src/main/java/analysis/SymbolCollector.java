    ...
    switch (decl) {
      case Declaration.FunctionDef func -> {
        Symbol.FunctionSymbol symbol =
            new Symbol.FunctionSymbol(
                func.name(), func.returnType(), func.parameters(), func.body());
        globalSymbols.define(symbol);
      }

      case Declaration.FunctionDecl func -> {
        Symbol.FunctionSymbol symbol =
            new Symbol.FunctionSymbol(func.name(), func.returnType(), func.parameters());
        globalSymbols.define(symbol);
      }
      ...
      case Declaration.ClassDef cls -> {
        ...
        for (ClassMember.FieldDecl field : cls.fields()) {
          Symbol.VariableSymbol fieldSymbol =
              new Symbol.VariableSymbol(
                  field.name(), field.type(), false // Felder können keine Referenzen sein
                  );
        ...
        for (ClassMember.ConstructorDef ctor : cls.constructors()) {
          Symbol.FunctionSymbol ctorSymbol =
              new Symbol.FunctionSymbol(
                  ctor.name(),
        ...
        for (ClassMember.MethodDef method : cls.methods()) {
          Symbol.FunctionSymbol methodSymbol =
              new Symbol.FunctionSymbol(
                  method.name(), method.returnType(), method.parameters(), method.body());
